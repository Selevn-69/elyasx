...
export default function DriverLoginPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      localStorage.setItem('driver_id', String(driver.driver_id))
      ...
      <main className="w-full max-w-[480px]">
        <div className="login-card bg-white rounded-xl p-xxl flex flex-col items-center">
          <div className="mb-xxl w-full flex justify-center">
            <img src="/logo.png" alt="ElyasX" className="w-36 h-auto object-contain mx-auto" />
          </div>
          <div className="w-full mb-xl text-center">
            <h1 className="font-h2 text-h2 text-on-background mb-sm">Welcome back, Driver</h1>
            <p className="font-body-md text-body-md text-on-surface-variant">Access your dashboard to start deliveries</p>
          </div>
          <form className="w-full space-y-lg" onSubmit={handleSubmit}>
            ...
            <div className="space-y-sm">
              ...
              <div className="relative">
                <span className="material-symbols-outlined absolute left-md top-1/2 -translate-y-1/2 text-outline">phone</span>
                <input
            ...
            <button
              ...
              disabled={loading}
            >
              <span>{loading ? 'Signing in...' : 'Login as Driver'}</span>
              {!loading && <span className="material-symbols-outlined text-[20px]">arrow_forward</span>}
            </button>
          </form>
          <div className="w-full mt-xxl pt-xl border-t border-surface-container flex flex-col gap-md">
            <p className="font-body-sm text-body-sm text-on-surface-variant text-center">
              Not a driver yet?{' '}
              <a className="text-primary font-label-bold hover:underline" href="#">
                Apply to join the fleet
              </a>
            </p>
            <div className="grid grid-cols-2 gap-md">
              <div className="bg-surface-container-low rounded-lg p-md flex items-center gap-md border border-transparent hover:border-outline-variant transition-all cursor-pointer">
                <div className="bg-primary-fixed w-10 h-10 rounded-full flex items-center justify-center">
                  <span className="material-symbols-outlined text-on-primary-container">support_agent</span>
                </div>
                <div>
                  <span className="block font-label-bold text-label-bold text-on-surface">Support</span>
                  <span className="block font-body-sm text-[12px] text-on-surface-variant">24/7 Help</span>
                </div>
              </div>
              <div className="bg-surface-container-low rounded-lg p-md flex items-center gap-md border border-transparent hover:border-outline-variant transition-all cursor-pointer">
                <div className="bg-tertiary-fixed w-10 h-10 rounded-full flex items-center justify-center">
                  <span className="material-symbols-outlined text-on-tertiary-fixed-variant">map</span>
                </div>
                <div>
                  <span className="block font-label-bold text-label-bold text-on-surface">Cities</span>
                  <span className="block font-body-sm text-[12px] text-on-surface-variant">Service Area</span>
                </div>
              </div>
            </div>
          </div>
